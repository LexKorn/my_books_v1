...
class BookController {
    async create(req, res, next) {
        try {
            let {name, link, rating, comment, userId, countryId, authorId} = req.body;
            const {cover} = req.files;
            let fileName = uuid.v4() + ".jpg";

            cover.mv(path.resolve(__dirname, '..', 'static', fileName));

            const book = await Book.create({name, link, rating, comment, userId, countryId, authorId, cover: fileName});            
    ...
    async getAll(req, res, next) {
        try {
            ...
            if (countryId && !authorId) {
                books = await Book.findAndCountAll({where:{countryId}, limit, offset});
            }
            ...
            if (countryId && authorId) {
                books = await Book.findAndCountAll({where:{countryId, authorId}, limit, offset});
            }
    ...
    async update(req, res, next) {
        try {
            ...
            await Book.update({name, link, rating, comment, cover}, {where: {id}});
